...
public class ClimberSubsystem extends SubsystemBase {
    ...
    public Command runClimberDown() {
        return climbCommand(() -> ClimberConstants.climberDownSpeed.get()).until(() -> getClimberPosition() <= 0);
    }
    //run Climber forward - climb up
    public Command runClimberUp() {
        return climbCommand(() -> ClimberConstants.climberUpSpeed.get()).until(() -> getClimberPosition() >= 2); //2 is example value
    }
    ...
    private void configureClimber() {
            ...
            climberConfig.SoftwareLimitSwitch.ForwardSoftLimitThreshold = 2.0; 
            climberConfig.SoftwareLimitSwitch.ReverseSoftLimitThreshold = 0.0; 
